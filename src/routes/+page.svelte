<script async>
	...
	const relays = ['wss://relay.snort.social', 'wss://relay.damus.io', 'wss://nostr.wine'];

	const events = eventStore(relays, customFilter);
	const messages = messageStore(publicKey, relays);
	const NostrSession = nostrSession(event, relays);
	const NostrProfile = nostrProfile(publicKey, relays);
</script>
